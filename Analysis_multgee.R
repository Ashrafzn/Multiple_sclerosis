...
# Creating a dataframe to save the probs of every sample
probabilities=data.frame(index=1:nrow(df))
...
Brier_scores=data.frame(index=1:length(names(comparisons_9types)))
rownames(Brier_scores)=names(comparisons_9types)

# Computing the Brier Score
for (the_type in names(comparisons_9types)){
  # Predictions vs the true labels
  predicted_probability = probabilities[,the_type]
  observed_label = comparisons_9types[[the_type]]$types
  Brier = mean((predicted_probability-observed_label)^2)
  
  # Predictions vs Random guessing
  number_of_lesions = sum(observed_label)
  guessing = sample(1:nrow(df), number_of_lesions, replace = F )
  rand_vec = rep(0,nrow(df))
  rand_vec[guessing] = 1
  Brier_random = mean((predicted_probability-rand_vec)^2)
  
  # Prediction vs Majority class
  if (the_type == as.character(which.max(table(df$types)))){
    maj_vec = rep(1,nrow(df))
  }
  else {
    maj_vec = rep(0,nrow(df))
  }
  Brier_majority = mean((predicted_probability-maj_vec)^2)
    
  Brier_scores[the_type,'models']=Brier
  Brier_scores[the_type,'Random_guessing']=Brier_random
  Brier_scores[the_type,'Majority_guessing']=Brier_majority
}

# Deleting the index column
Brier_scores=Brier_scores[,-1]

# Obtaining the final result by averaging over all types.
Brier_results = rbind(Brier_scores,apply(Brier_scores,2,mean ))
rownames(Brier_results)[nrow(Brier_results)]='Result(mean value)'
